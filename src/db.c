...
#define DB_VER_TABLE_LOG         ("VERBOSETABELELOG")
...
db_dump_meta(struct DB *const db) {
    ...
    for (int i = 0; db->cms_dump[i]; i++) {
        ...
        sprintf(path_cm_dump, "%s/%s/%s-%01d-%018.6lf", db->persist_dir, DB_META_BACKUP_DIR, DB_META_CMAP_PREFIX, i,
                sec0);
...
db_table_dump(struct DB *const db, struct Table *const table, const uint64_t start_bit) {
    ...
    if (rr == false) {
//        char path[4096];
//        sprintf(path, "%s/%s", db->persist_dir, DB_VER_TABLE_LOG);
//        FILE *const verblogfile = fopen(path, "a"); // NULL is OK
//        table_analysis_verbose(table, verblogfile);
//        fclose(verblogfile);
