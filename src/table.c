...
__hash_ht(const uint8_t *const hash) {
    ...
    const uint32_t hv = (*phv) % ITEMS_PER_BARREL;
    return hv;
}
...
item_insert(struct Item **const items, struct Item *const item) {
    ...
    const uint16_t victim_size = item_erase(items, item);
    // insert in front
    item->next = *items;
    (*items) = item;
    return victim_size;
...
barrel_count(struct Barrel *const barrel) {
    uint16_t count = 0;
    for (uint64_t i = 0; i < ITEMS_PER_BARREL; i++) {
...
barrel_erase(struct Barrel *const barrel, struct Item *const item) {
    ...
    const uint16_t size = item_erase(&(barrel->items[hid]), item);
    barrel->current_size -= size;
}

static inline void
barrel_insert(struct Barrel *const barrel, struct Item *const item) {
    const uint32_t hid = item_hash_ht(item);
    const uint16_t victim_size = item_insert(&(barrel->items[hid]), item);
    barrel->current_size += item->current_size;
    barrel->current_size -= victim_size;
}
...
table_initial(struct Table *const table, const uint64_t capacity) {
    ...
    if (table->io_buffer == NULL) {
        table->io_buffer = huge_alloc(BARREL_SIZE * TABLE_NR_IO);
...
table_free(struct Table *const table) {
    ...
    if (table->io_buffer) {
        huge_free(table->io_buffer, BARREL_SIZE * TABLE_NR_IO);
    }
    if (table->bt) {
...
table_full(const struct Table *const table) {
    return (table->current_size >= table->capacity) ? true : false;
}
...
table_insert_item_mt(struct Table *const table, struct Item *const item) {
    ...
    barrel_insert(barrel, item);
    const uint16_t vol1 = barrel->current_size;
    __sync_add_and_fetch(&(table->current_size), (vol1 - vol0));
...
static inline int
__compare_size(const void *const p1, const void *const p2) {
...
static void
retaining_sort_barrels_by_size(struct Table *const table, struct Barrel **barrels) {
    for (uint64_t i = 0; i < BARRELS_PER_TABLE; i++) {
        barrels[i] = &(table->barrels[i]);
    }
    qsort(barrels, BARRELS_PER_TABLE, sizeof(barrels[0]), __compare_size);
}
...
table_retain(struct Table *const table) {
    ...
    while (true) {
        ...
        retaining_sort_barrels_by_size(table, barrels);
...
table_dump_barrels(struct Table *const table, const int fd, const uint64_t off) {
    ...
    for (uint64_t j = 0; j < BARRELS_PER_TABLE; j += TABLE_NR_IO) {
        ...
        if (nr_dump < TABLE_NR_IO) {
            bzero(&(table->io_buffer[BARREL_SIZE * nr_dump]), BARREL_SIZE * (TABLE_NR_IO - nr_dump));
        }
        const size_t nr_bytes = (size_t) (TABLE_NR_IO * BARREL_SIZE);
        const uint64_t off_j = off + (BARREL_SIZE * j);
...
table_analysis_short(struct Table *const table, char *const buffer) {
    ...
    const double vp = ((double) table->current_size) * 100.0 / ((double) TABLE_SIZE);
    ...
    sprintf(buffer, "%8"PRIu64" (%5.2lf%%) %4"PRIu64" (%.1lfKB) %7"PRIu32" (%.1lfKB)",
            table->current_size, vp, table->nr_mi, ik, bt_bytes, bk);
}
...
raw_barrel_fetch(struct MetaTable *const mt, const uint64_t barrel_id, uint8_t *const buf) {
    const uint64_t off_barrel = (barrel_id * BARREL_SIZE) + mt->mfh.off;
    const ssize_t r = pread(mt->raw_fd, buf, BARREL_SIZE, (off_t) off_barrel);

    if (mt->stat) {
        __sync_add_and_fetch(&(mt->stat->nr_fetch_barrel), 1);
    }
    return (r == BARREL_SIZE) ? true : false;
}

static bool
raw_barrel_fetch_multiple(struct MetaTable *const mt, const uint64_t start_id,
                          const uint64_t nbarrels, uint8_t *const buf) {
    const uint64_t off_barrel = (start_id * BARREL_SIZE) + mt->mfh.off;
    const size_t bytes = BARREL_SIZE * nbarrels;
...
metatable_lookup(struct MetaTable *const mt, const uint16_t klen,
    ...
    uint8_t *buf = aligned_alloc(BARREL_SIZE, BARREL_SIZE);
