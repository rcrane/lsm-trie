...
static struct DBParams pstable[] = {
        ...
        {"Dummy", 502, "lsmtrie_tmp", "cm_conf1.txt", 1, 100, "uniform", UINT64_C(100000000000), 50, 100000},
};
...
main(int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv,
        ...
        switch (opt) {
            ...
            case 'v':
                ps.vlen = strtoull(optarg, NULL, 10);
                break;
            case 'a':
                ps.nr_threads = strtoull(optarg, NULL, 10);
                break;
            case 'w':
                ps.p_writer = strtoull(optarg, NULL, 10);
                break;
            case 't':
                ps.sec = strtoull(optarg, NULL, 10);
                break;
            case 'n':
                ps.nr_report = strtoull(optarg, NULL, 10);
                break;
            case 'r':
                ps.range = strtoull(optarg, NULL, 10);
                break;

            case 'd':
                ps.meta_dir = strdup(optarg);
                break;
            case 'c':
                ps.cm_conf_fn = strdup(optarg);
                break;
            case 'g':
                ps.generator = strdup(optarg);
                break;
            case 'h': {
                show_dbparams(&ps);
                exit(1);
